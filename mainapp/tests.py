...
class ShopTestCases(TestCase):
    ...
    def test_add_to_cart(self):
        ...
        self.assertIn(self.cart_product, self.cart.products.all()) # проверяет вхождение чего-то в каком-либо контейнере
        self.assertEqual(self.cart.products.count(),1) # кол-во объектов в корзине одно, нет дубляжа
        self.assertEqual(self.cart.final_price, Decimal('50000.00')) # стоимость столько, сколько мы указали


    # Проверяем методы добавления в карзину через представления
